...
public:
	 ...
	 * @param pl игрок
	 * @param ent список всех предметов, которые появляются в игре много раз.
	...
	/*
	 * Отвечает за прокрутку экрана и передвижение платформ по мере движения игрока наверх.
	 * @param pl игрок
	 * @param ent список всех предметов, которые появляются в игре много раз.
	 * @param it итератор для обхода по списку.
	 */
	void Move(Player &pl, std::list<Entity*> &ent, std::list<Entity*>::iterator &it) {
		...
		{
			for (it = ent.begin(); it != ent.end(); it++) {
				pl.body.setPosition({ pl.body.getPosition().x, pl.h }); // Координата У установлена на 200
	 ...
	 * @param ent список всех предметов, которые появляются в игре много раз.
	 * @param it итератор для обхода по списку.
	 * @param window окно, где происходит действите.
	 */
	...
	/*
	 * Процесс игры. Вызывается функция передвижения, рисования, обрабатываются столкновения.
	 * @param pl игрок
	 * @param ent список всех предметов, которые появляются в игре много раз.
	 * @param it итератор для обхода по списку.
	 * @param window окно, где происходит действите.
